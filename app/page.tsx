...
import { getAllOcorrencias, getStats, type OcorrenciaComEvidencias } from './actions';
                ...
                <tbody className="divide-y divide-gray-100">
                  {ocorrencias.map((o) => (
                    <tr key={o.id} className="hover:bg-gray-50 transition-colors cursor-pointer group">
                      ...
                      <td className="px-6 py-4">
                        {o.total_evidencias > 0 ? (
                          <Link
                            href={`/ocorrencia/${o.id}`}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-brand-200 bg-brand-50 text-brand-600 hover:bg-brand-100 transition-colors text-xs font-medium"
                          >
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                            Visualizar
                            <span className="bg-brand-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px] font-bold">{o.total_evidencias}</span>
                          </Link>
                        ) : (
